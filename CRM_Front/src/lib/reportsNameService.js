const REPORTS_NAME_API_URL = `${import.meta.env.VITE_API_BASE_URL}/api/reports_names`;
...
const reportsNameService = {
  async getReportsNames(orgId) {
    const token = checkToken();

    try {
      const url = new URL(REPORTS_NAME_API_URL);
      ...
      console.error('Error fetching reports names:', error);
      throw error;
    }
  },

  async getReportName(id, orgId) {
    const token = checkToken();

    try {
      const url = new URL(`${REPORTS_NAME_API_URL}/${id}`);
      url.searchParams.append('org_id', orgId);
  ...
  async createReportName(data) {
    const token = checkToken();

    try {
      const response = await fetch(REPORTS_NAME_API_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-access-tokens': token,
      ...
      if (response.status === 401) {
        try {
          return await refreshTokenAndRetry(REPORTS_NAME_API_URL, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'x-access-tokens': token,
      ...
      console.error('Error creating report name:', error);
      throw error;
    }
  },

  async updateReportName(id, data) {
    const token = checkToken();
  ...
  async deleteReportName(id) {
    ...
    try {
      ...
      console.error('Error deleting report name:', error);
      throw error;
    }
  },
};

export { reportsNameService };