...
const investigateReportService = {
  async getReports(orgId, { page = 1, perPage = 10, reportType = '', sort = '-created_at', lang = 'en' }) {
    ...
    try {
      ...
      url.searchParams.append('status', 'Escalated'); // Filter for Escalated reports
  ...
  async updateInvestigationDetails({ id, orgId, investigationDetails, accessToken }) {
    try {
      const url = new URL(`${MONGO_API_URL}/api/reports/${id}/investigation-details`);
      url.searchParams.append('org_id', orgId);

      const requestBody = { investigation_details: investigationDetails };
      console.log('Sending investigation details update request:', { url: url.toString(), body: requestBody });
      ...
      try {
        ...
        console.error('Error parsing response JSON:', jsonError);
        throw new Error(`Failed to update investigation details: Invalid response format (${response.status})`);
      }
      ...
      if (!response.ok) {
        console.error('Investigation details update failed:', {
          status: response.status,
          statusText: response.statusText,
          responseData,
          id,
        ...
        throw new Error(responseData.message || `Failed to update investigation details: ${response.statusText} (${response.status})`);
      }

      console.log('Investigation details update response:', responseData);
      return responseData;
    } catch (error) {
...
export default investigateReportService;