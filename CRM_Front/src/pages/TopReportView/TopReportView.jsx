...
//   // Define current date for duration calculation and date violation check
//   const currentDate = new Date('2025-06-07T16:36:00.000Z'); // 4:36 PM EEST (UTC+3)

//   // Format corrective action date if it exists
...
//               {t('reports.status') || 'Status'}: {t(`reports.status_${report.status.toLowerCase().replace(' ', '_')}`) || report.status}
//             </p>
//           )}
//           {report.severity && (
//             <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
...
//             </p>
//           )}
//           {(report.corrective_action_required && (report.corrective_action_description || report.corrective_action_date)) || report.investigation_details || report.management_notes ? (
//             <div className="mb-4">
//               {report.corrective_action_required && (report.corrective_action_description || report.corrective_action_date) && (
//                 <div className="mb-4">
//                   <h4 className="text-md font-medium text-gray-800 dark:text-gray-100 mb-2">
...
//                   )}
//                   {report.corrective_action_date && (
//                     <p className={`text-sm ${isDateViolation ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-gray-400'} mb-2`}>
//                       {isDateViolation
...
// export default TopReportView;



// Working 1000 befor tables 
...
const ReportModal = ({ report, onClose, authData, t }) => {
  ...
  const currentDate = new Date('2025-06-07T16:36:00.000Z');
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredReports.map((report) => {
                          ...
                          return (
                            ...
                            >
                              {report.status && (
                                <div className={`${getStatusBarClass(report.status)} text-white text-sm font-bold text-center py-2 rounded-t-lg -mt-6 -mx-6 mb-4`}>
                                  {t(`reports.status_${report.status.toLowerCase().replace(' ', '_')}`) || report.status}
                                </div>
                              )}
                              <div className="space-y-3">
