...
const ReportsTable = ({ reports, signatureUrls, t, setSelectedReport, selectedColumns, getFieldValue }) => {
  ...
  const filteredReports = reports.filter((report) => {
    ...
    if (columnFilters.created_at) {
      matches = reports && (report.created_at ? format(new Date(report.created_at), 'PPP') : '').toLowerCase().includes(columnFilters.created_at.toLowerCase());
    }
...
const DynamicReportAdvanced = () => {
  ...
  const [doughnutXAxis, setDoughnutXAxis] = useState('qa_section');
  const [doughnutYAxis, setDoughnutYAxis] = useState('');
  const [barXAxis, setBarXAxis] = useState('qa_section');
  const [barYAxis, setBarYAxis] = useState('');
  const [tableXAxis, setTableXAxis] = useState('qa_section');
  const [tableYAxis, setTableYAxis] = useState('');
  ...
  // Get field value for dynamic columns
  ...
  const getChartData = (reports, xAxis, yAxis) => {
    const dataMap = reports.reduce((acc, report) => {
      let xValue = '';
      if (xAxis === 'user_created_name') xValue = report.user_created_name || 'Unknown';
      else if (xAxis === 'main_location_name') xValue = report.main_location_name || report.structure?.location_details?.main_location_id || 'Unknown';
      else if (xAxis === 'qa_section') xValue = report.qa_section || report.structure?.location_details?.section_qa_id || 'Unknown';
      else if (xAxis === 'qa_sub_section') xValue = report.qa_sub_section || report.structure?.location_details?.sub_section_qa_id || 'Unknown';
      else if (xAxis === 'created_at') xValue = report.created_at ? format(new Date(report.created_at), 'PPP') : 'Unknown';

      const yValue = yAxis ? getFieldValue(report, allColumns.find(col => col.key === yAxis)?.name) || 'Unknown' : 'Count';
    ...
    const sortedData = Object.entries(dataMap)
      .map(([key, count]) => {
        ...
        return { xValue, yValue, count };
      })
      .sort((a, b) => b.count - a.count)
      .slice(0, 10);

    const labels = sortedData.map(item => `${item.xValue} (${item.yValue})`);
    const data = sortedData.map(item => item.count);

    console.log('Chart data - X:', xAxis, 'Y:', yAxis, 'Labels:', labels, 'Data:', data);
    ...
    return {
      ...
      bar: {
        ...
        datasets: [
          {
            label: translate('reports.count', 'Report Count'),
            data,
            backgroundColor: chartColors,
      ...
      table: sortedData.map((item, index) => ({
        rank: index + 1,
        xValue: item.xValue,
        yValue: item.yValue,
        count: item.count,
  ...
  // Update report names and columns when reports change
  useEffect(() => {
    if (reports.length > 0) {
      console.log('Reports updated:', reports);
      const uniqueNames = [...new Set(reports.map(report => report.name && report.name.trim()))].filter(Boolean);
      console.log('Unique report names:', uniqueNames);
      setReportNames(uniqueNames);
      if (uniqueNames.length > 0 && !selectedReportName) {
        setSelectedReportName(uniqueNames[0]);
      }
    } else {
      console.log('No reports, setting reportNames to empty');
      setReportNames([]);
      setSelectedReportName('');
    }
  }, [reports]);

  // Update columns based on selected report name
  useEffect(() => {
    if (selectedReportName && reports.length > 0) {
      const selectedReport = reports.find(report => report.name && report.name.trim() === selectedReportName);
      if (selectedReport) {
        console.log('Selected report for columns:', selectedReport);
        const structureFields = getStructureFields(selectedReport);
        ...
        if (structureFields.length > 0) {
          setDoughnutYAxis(structureFields[0].key);
          setBarYAxis(structureFields[0].key);
          setTableYAxis(structureFields[0].key);
        } else {
          setDoughnutYAxis('');
          setBarYAxis('');
          setTableYAxis('');
        ...
        console.log('No report found for selected name, setting allColumns to fixedColumns');
      ...
      console.log('No selected report name or reports, setting allColumns to fixedColumns');
      setAllColumns(fixedColumns);
      setSelectedColumns(fixedColumns.map(col => col.key));
      setTempSelectedColumns(fixedColumns.map(col => col.key));
      setDoughnutYAxis('');
  ...
  }, [selectedReportName, reports, language, t]);
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      setTotalPages(Math.ceil(allReports.length / perPage));
  ...
  useEffect(() => {
    ...
    if (selectedReportName) {
      filtered = reports.filter(report => report.name && report.name.trim() === selectedReportName);
      console.log('Filtered reports by selected report name:', filtered);
    }
    ...
    console.log('Setting totalPages:', Math.ceil(filtered.length / perPage));
    setTotalPages(Math.ceil(filtered.length / perPage));
    setFilteredReports(filtered.slice((currentPage - 1) * perPage, currentPage * perPage));
  }, [reports, searchQuery, filters.startDate, filters.endDate, currentPage, perPage, selectedReportName]);
  ...
  const handlePerPageChange = (e) => {
    ...
    console.log('Items per page changed:', newPerPage);
  ...
  const handleReportNameChange = (e) => {
    ...
    console.log('Selected report name:', name);
  ...
  const toggleGraphs = () => {
    console.log('Graphs Views button clicked, showGraphs:', !showGraphs);
    setShowGraphs(!showGraphs);
  };

  const handleDoughnutXAxisChange = (e) => {
    console.log('Doughnut X-axis changed:', e.target.value);
    setDoughnutXAxis(e.target.value);
  };

  const handleDoughnutYAxisChange = (e) => {
    console.log('Doughnut Y-axis changed:', e.target.value);
    setDoughnutYAxis(e.target.value);
  };

  const handleBarXAxisChange = (e) => {
    console.log('Bar X-axis changed:', e.target.value);
    setBarXAxis(e.target.value);
  };

  const handleBarYAxisChange = (e) => {
    console.log('Bar Y-axis changed:', e.target.value);
    setBarYAxis(e.target.value);
  };

  const handleTableXAxisChange = (e) => {
    console.log('Table X-axis changed:', e.target.value);
    setTableXAxis(e.target.value);
  };

  const handleTableYAxisChange = (e) => {
    console.log('Table Y-axis changed:', e.target.value);
    setTableYAxis(e.target.value);
  };
  ...
  const dynamicColumns = allColumns.filter(col => !fixedColumns.some(fc => fc.key === col.key));
  const doughnutData = getChartData(filteredReports, doughnutXAxis, doughnutYAxis);
  const barData = getChartData(filteredReports, barXAxis, barYAxis);
  const tableData = getChartData(filteredReports, tableXAxis, tableYAxis);
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            <div className="sm:flex sm:justify-between sm:items-center mb-8">
              <h1 className="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">
                {translate('reports.view_reports', 'View Reports')}
              </h1>
            ...
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <button
                ...
                className="px-4 py-2 bg-teal-500 text-white rounded-lg hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-500 transition duration-200"
                aria-label={translate('reports.graphs_view', 'Graphs View')}
              >
                {translate('reports.graphs_view', 'Graphs View')}
              </button>
            </div>
                ...
                >
                  <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">
                      {translate('reports.doughnut_chart', 'Report Distribution')}
                    </h3>
                    <div className="flex gap-4 mb-4">
                      <div className="flex-1">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                          {translate('reports.x_axis', 'X-Axis')}
                        </label>
                        <select
                          value={doughnutXAxis}
                          onChange={handleDoughnutXAxisChange}
                          className="w-full py-2 px-3 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        >
                      ...
                      <div className="flex-1">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                          {translate('reports.y_axis', 'Y-Axis')}
                        </label>
                        <select
                          value={doughnutYAxis}
                          onChange={handleDoughnutYAxisChange}
                          className="w-full py-2 px-3 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          disabled={dynamicColumns.length === 0}
                        >
                          <option value="">{translate('reports.select_y_axis', 'Select Y-Axis')}</option>
                    ...
                    {doughnutData.doughnut.labels.length > 0 ? (
                      <Doughnut
                        data={doughnutData.doughnut}
                        options={{
                          responsive: true,
                          plugins: {
                            legend: { position: 'top', labels: { color: language === 'ar' ? '#E5E7EB' : '#1F2937' } },
                  ...
                  <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">
                      {translate('reports.bar_chart', 'Reports by Category')}
                    </h3>
                    <div className="flex gap-4 mb-4">
                      <div className="flex-1">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    ...
                    {barData.bar.labels.length > 0 ? (
                      <Bar
                        ...
                        options={{
                          ...
                          scales: {
                            y: { beginAtZero: true, ticks: { color: language === 'ar' ? '#E5E7EB' : '#1F2937' } },
                  ...
                  <div className="md:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">
                      {translate('reports.top_sections', 'Top 10 by Count')}
                    </h3>
                    <div className="flex gap-4 mb-4">
                      <div className="flex-1">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                        ...
                        <select
                          value={tableXAxis}
                          onChange={handleTableXAxisChange}
                          className="w-full py-2 px-3 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        >
                          {fixedColumns.map((column) => (
                            <option key={column.key} value={column.key}>
                      ...
                      <div className="flex-1">
                        ...
                        <select
                          value={tableYAxis}
                          onChange={handleTableYAxisChange}
                          className="w-full py-2 px-3 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          disabled={dynamicColumns.length === 0}
                        >
                          <option value="">{translate('reports.select_y_axis', 'Select Y-Axis')}</option>
                      ...
                      <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                          <thead className="bg-teal-50 dark:bg-teal-900">
                            <tr>
                              <th className="px-6 py-3 text-left text-xs font-medium text-teal-600 dark:text-teal-300 uppercase tracking-wider">
                                {translate('reports.rank', 'Rank')}
                              </th>
                              <th className="px-6 py-3 text-left text-xs font-medium text-teal-600 dark:text-teal-300 uppercase tracking-wider">
                                {translate('reports.x_axis', 'X-Axis')}
                              </th>
                              ...
                              <th className="px-6 py-3 text-left text-xs font-medium text-teal-600 dark:text-teal-300 uppercase tracking-wider">
                                {translate('reports.count', 'Report Count')}
                              </th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {tableData.table.map((row) => (
                              <tr key={`${row.xValue}-${row.yValue}`} className="hover:bg-teal-50 dark:hover:bg-teal-900 transition-colors duration-200">
                                <td className="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">{row.rank}</td>
                                <td className="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">{row.xValue}</td>
                                <td className="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">{row.yValue}</td>
                                <td className="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">{row.count}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
            ...
            {filteredReports.length === 0 && !loading ? (
              <div className="text-center text-gray-600 dark:text-gray-300 py-12">
                ...
                <p className="text-lg">{translate('reports.no_reports', 'No reports found')}</p>
              </div>
            ) : (
              <>
                <ReportsTable
                ...
                <div className="flex justify-center items-center gap-3 mt-8">
                  ...
                  >
                    {[5, 10, 15, 50, 75, 100, 200, 1000,10000,100000,1000000].map((value) => (
...
export default DynamicReportAdvanced;