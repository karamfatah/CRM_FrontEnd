...
// // RawInspectionPage Component
// const RawInspectionPage = () => {
...
//   const [perPage, setPerPage] = useState(10000000);
...
//     endDate: '',
//   });
//   const [selectedReportName] = useState('Raw material rejection form || نموذج رفض خامات');
//   const [reportNames] = useState(['Raw material rejection form || نموذج رفض خامات']);
...
//       // Filter reports to only include those with the specified name
//       const filteredReports = allReports.filter(report => report.name && report.name.trim() === 'Raw material rejection form || نموذج رفض خامات');
//       console.log('Fetched and filtered reports:', filteredReports);
//       setReports(filteredReports);
//       setTotalPages(Math.ceil(filteredReports.length / perPage));
...
//                 disabled
//                 className="w-full py-2 px-3 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 opacity-50 cursor-not-allowed"
//               >
//                 <option value="Raw material rejection form || نموذج رفض خامات">
//                   Raw material rejection form || نموذج رفض خامات
//                 </option>
//               </select>
//             </div>
//             <div className="mb-6">
...
//                   >
//                     {[5, 10, 15, 50, 75, 100, 200, 1000].map((value) => (
...
// export default RawInspectionPage;
...
import { PrevArrow, NextArrow, ImageModal, ImageCarousel, ReportModal } from './RawInspectionPageHelpers';
...
const RawInspectionPage = () => {
  ...
  useEffect(() => {
    ...
    if (hasPrivilegeCheck([5000, 1,1011])) {
      console.log('User has privilege, fetching reports');
      setHasPrivilege(true);
      fetchReports();
    } else {
  ...
  if (!hasPrivilegeCheck([5000, 1, 1006])) {
    return (
      <div className="flex h-screen overflow-hidden">
        <Sidebar sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
        <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
          ...
          <main>
            <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
              <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg shadow-sm relative mb-6" role="alert">
                <span>{translate('reports.no_permission', 'No permission to view reports')}</span>
              </div>
            </div>
          </main>
        </div>
