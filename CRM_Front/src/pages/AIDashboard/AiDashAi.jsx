...
// =============== flash util (for KPIs, used later) ===============
const useFlash = (value, duration = 900) => {
...
const AiDashAi = () => {
  ...
  // FLASH on KPI change
  const flashOpen = useFlash(kpis.open);
  const flashNoMove = useFlash(kpis.noMove);
  const flashBreached = useFlash(kpis.breached);
  const flashResolved = useFlash(kpis.resolvedToday);
  ...
  // FLASH on quality KPI change
  const flashThroughput = useFlash(qualityStats.throughput7d);
  const flashNotes = useFlash(qualityStats.notesPct);
  const flashSla = useFlash(qualityStats.slaPct);
  ...
  // 9) Issue Types × Destination Departments (Matrix) — hook
  const issueDeptMatrix = useMemo(() => {
    const rptIds = new Set(actionsFiltered.map(a => a.report_id));
    const filteredReports = reports.filter(r => rptIds.has(normalizeId(r)));
  ...
  // Clickable Issue×Dept cell drilldown
  const onIssueDeptCellClick = (issue, dept) => {
    const rptIds = new Set(actionsFiltered.map(a => a.report_id));
    const reps = reports.filter(r => {
  ...
  // fetch single report (AI-only) when selecting from drilldown
  const handleSelectReport = useCallback(async (r) => {
    try {
      const rid = normalizeId(r);
      const res = await aiDashboardService.fetchAIReportByIdAIOnly(
        authData.org_id,
        rid,
        authData.access_token
      );
        ...
        <main>
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className={`bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-gray-200 dark:border-gray-700 ${flashOpen ? 'ring-2 ring-amber-400 ring-offset-2 ring-offset-white dark:ring-offset-gray-800 animate-pulse' : ''}`}>
              ...
              <div className={`bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-gray-200 dark:border-gray-700 ${flashNoMove ? 'ring-2 ring-gray-400 ring-offset-2 ring-offset-white dark:ring-offset-gray-800 animate-pulse' : ''}`}>
              ...
              <div className={`bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-gray-200 dark:border-gray-700 ${flashBreached ? 'ring-2 ring-red-400 ring-offset-2 ring-offset-white dark:ring-offset-gray-800 animate-pulse' : ''}`}>
              ...
              <div className={`bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-gray-200 dark:border-gray-700 ${flashResolved ? 'ring-2 ring-green-400 ring-offset-2 ring-offset-white dark:ring-offset-gray-800 animate-pulse' : ''}`}>
            ...
            {/* QUALITY KPIs with flash */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
              <div className={`bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-gray-200 dark:border-gray-700 ${flashThroughput ? 'ring-2 ring-sky-400 ring-offset-2 ring-offset-white dark:ring-offset-gray-800 animate-pulse' : ''}`}>
              ...
              <div className={`bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-gray-200 dark:border-gray-700 ${flashNotes ? 'ring-2 ring-indigo-400 ring-offset-2 ring-offset-white dark:ring-offset-gray-800 animate-pulse' : ''}`}>
              ...
              <div className={`bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-gray-200 dark:border-gray-700 ${flashSla ? 'ring-2 ring-emerald-400 ring-offset-2 ring-offset-white dark:ring-offset-gray-800 animate-pulse' : ''}`}>
            ...
            {/* Row 4: Issue Types × Destination Departments (CLICKABLE) */}
            <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 mb-8 overflow-x-auto">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3">
                      ...
                      {issueDeptMatrix.cols.map((ck) => {
                        ...
                        return (
                          <td
                            ...
                            onClick={() => v > 0 && onIssueDeptCellClick(rk, ck)}
            ...
            {/* -- Part 3 continues with Needs Attention table, DrilldownPanel mount, ReportModal mount, AttentionTable, and export -- */}
